...
def build_datasets_index():
    ...
    # 2. create table and place in api/datasbase.rst
    table = info_table(entries)

    file_contents = (_DOC_SOURCE / "api" / "database.rst").read_text()
    lines = file_contents.split("\n")
    start_idx = lines.index(".. DATABASES-START") + 1
    end_idx = lines.index(".. DATABASES-END")
    lines = lines[:start_idx] + table.split("\n") + lines[end_idx:]
    (_DOC_SOURCE / "api" / "database.rst").write_text("\n".join(lines))
...
def info_table(entries: list[DatabaseEntry]) -> str:
    header = """\
.. This file is autogenerated by dev/scripts/generate_page.py::info_table
